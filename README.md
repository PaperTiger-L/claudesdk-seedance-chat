...
通过自然语言对话生成专业 Seedance 2.0 视频脚本和分镜提示词的 Web 应用。基于 Claude Agent SDK 构建，支持 **短剧模式** 与 **广告模式** 两种创作路径，可用于故事改编、短剧分镜、带货视频、品牌 TVC 和产品广告提示词生成。
...
如果本地没有 Docker、但服务器有 Docker，推荐直接在服务器上部署。
...
```text
http://你的服务器IP:3003/api/health
```

### 5. 持久化目录

Docker Compose 已挂载以下目录为卷：
- `/app/output` — AI 生成文件
- `/app/uploads` — 上传素材
- `/app/logs` — 服务日志
