...
function ConversationSidebar({
  ...
  const handleRowClick = useCallback((event: MouseEvent<HTMLButtonElement>, record: ConversationRecord) => {
    ...
    onSelect(record.id);
    onCloseMobile();
  }, [anchorId, onCloseMobile, onSelect, records]);
  ...
  if (collapsed) {
    return (
      <aside className="hidden md:flex w-[74px] shrink-0 flex-col border-r border-slate-200/80 bg-white/85 px-3 py-4 backdrop-blur-md shadow-[10px_0_30px_rgba(15,23,42,0.05)]">
  ...
  const panel = (
    <aside className="flex h-full w-[88vw] max-w-[360px] shrink-0 flex-col border-r border-slate-200/80 bg-white/95 backdrop-blur-md shadow-[10px_0_30px_rgba(15,23,42,0.12)] md:w-[360px]">
      <div className="border-b border-slate-200/80 px-4 py-4 space-y-3">
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={onCloseMobile} className="rounded-full border border-slate-200 bg-white px-3 py-1.5 text-xs font-medium text-slate-600 md:hidden">关闭</button>
            <button onClick={onToggleCollapse} className="hidden rounded-full border border-slate-200 bg-white px-3 py-1.5 text-xs font-medium text-slate-600 md:inline-flex">收起</button>
          </div>
        </div>
            ...
            >
              <div className="flex items-start gap-3">
                <button
                  ...
                  className="mt-1 flex h-5 w-5 shrink-0 items-center justify-center rounded-md border border-slate-300 bg-white text-[10px] text-sky-600"
                >
                  {isSelected ? "✓" : ""}
                </button>
                <button onClick={() => { onSelect(record.id); onCloseMobile(); }} className="min-w-0 flex-1 text-left">
                  <div className="flex items-center gap-2">
              ...
              <div className="mt-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                <span className="text-[11px] text-slate-400">手机端可单选，桌面端支持 Shift/Ctrl 多选</span>
                <button onClick={() => handleDelete([record.id])} className="self-start rounded-full bg-rose-50 px-3 py-1 text-[11px] font-medium text-rose-600 hover:bg-rose-100 sm:self-auto">删除</button>
              </div>
            </div>
          );
        })}
    ...
    <>
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-40 flex md:hidden">
          <button className="flex-1 bg-slate-900/30 backdrop-blur-[1px]" onClick={onCloseMobile} aria-label="关闭侧边栏" />
          <div className="relative z-50">{panel}</div>
        </div>
      )}
    </>
  );
...
function MessageBubble({ msg }: { msg: ChatMessage }) {
  if (msg.role === "user") {
    return (
      <div className="mb-3 flex justify-end sm:mb-4">
        <div className="max-w-[88%] rounded-[20px] rounded-br-md bg-gradient-to-br from-sky-500 to-blue-600 px-3 py-2.5 text-sm text-white shadow-lg shadow-sky-200/70 whitespace-pre-wrap sm:max-w-[80%] sm:rounded-[24px] sm:px-4 sm:py-3">
          {msg.content}
          {msg.files?.map((file) => (
  ...
  if (msg.role === "system" && msg.toolCall) {
    ...
    return (
      <div className="mb-3 flex justify-start">
        <div className={`max-w-[90%] rounded-[20px] border px-3 py-2.5 text-sm shadow-sm sm:max-w-[85%] sm:rounded-[24px] sm:px-4 sm:py-3 ${toolCall.status === "running" ? "border-amber-200 bg-amber-50 text-amber-800" : "border-emerald-200 bg-emerald-50 text-emerald-800"}`}>
  ...
  if (msg.role === "system") {
    return (
      <div className="mb-3 flex justify-center">
        <div className="rounded-full border border-rose-200 bg-rose-50 px-3 py-1.5 text-xs text-rose-700 shadow-sm sm:px-4 sm:py-2 sm:text-sm">{msg.content}</div>
      </div>
    );
  }
  ...
  return (
    <div className="mb-3 flex justify-start sm:mb-4">
      <div className="max-w-[90%] rounded-[20px] rounded-bl-md border border-slate-200 bg-white px-4 py-3 text-slate-800 shadow-sm prose prose-slate prose-sm max-w-none sm:max-w-[85%] sm:rounded-[24px] sm:px-5 sm:py-4">
        <ReactMarkdown
          components={{
            table: ({ children }) => (
...
function WelcomeScreen({ mode, onSelect }: { mode: ChatMode; onSelect: (text: string) => void }) {
  ...
  return (
    <div className="flex h-full flex-col items-center justify-center gap-5 px-4 text-center text-slate-500 sm:gap-6 sm:px-6">
      <div className="flex h-16 w-16 items-center justify-center rounded-[24px] bg-white text-3xl shadow-lg shadow-slate-200/70 sm:h-20 sm:w-20 sm:rounded-[28px] sm:text-4xl">{meta.icon}</div>
      <div>
        <p className="text-xl font-semibold text-slate-900 sm:text-2xl">SeedanceChat · {meta.title}</p>
        <p className="mt-2 text-sm text-slate-500">选择模板快速开始，或直接描述你的创意需求。</p>
      </div>
      <div className="grid w-full max-w-3xl grid-cols-1 gap-3 sm:gap-4 md:grid-cols-2">
        {templates.map((template) => (
          <button
            key={template.title}
            onClick={() => onSelect(template.prompt)}
            className="rounded-[24px] border border-slate-200 bg-white p-4 text-left shadow-sm transition hover:-translate-y-0.5 hover:border-slate-300 hover:shadow-lg hover:shadow-slate-200/70 sm:rounded-[28px] sm:p-5"
          >
            <div className="text-2xl">{template.icon}</div>
            <p className="mt-3 text-base font-semibold text-slate-900 sm:mt-4">{template.title}</p>
    ...
    <div className="min-h-screen bg-[radial-gradient(circle_at_top_left,_rgba(14,165,233,0.12),_transparent_28%),radial-gradient(circle_at_top_right,_rgba(99,102,241,0.10),_transparent_22%),linear-gradient(180deg,#f8fbff_0%,#f1f5f9_48%,#eef2ff_100%)] text-slate-900">
      <div className="flex h-screen overflow-hidden">
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          <header className="border-b border-white/70 bg-white/80 px-4 py-3 backdrop-blur-md shadow-sm shadow-slate-200/40 sm:px-6 sm:py-4">
            <div className="flex items-start justify-between gap-3 sm:items-center sm:gap-4">
              <div className="min-w-0 flex items-start gap-3 sm:items-center">
                <button onClick={() => setMobileSidebarOpen(true)} className="mt-1 flex h-10 w-10 items-center justify-center rounded-2xl border border-slate-200 bg-white text-slate-700 shadow-sm md:hidden">☰</button>
                <button onClick={() => setSidebarCollapsed((prev) => !prev)} className="hidden h-10 w-10 items-center justify-center rounded-2xl border border-slate-200 bg-white text-slate-700 shadow-sm md:flex">☰</button>
                <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-slate-900 text-lg text-white shadow-lg shadow-slate-300/30 sm:h-11 sm:w-11 sm:text-xl">{modeMeta.icon}</div>
                <div className="min-w-0">
                  <h1 className="truncate text-lg font-semibold tracking-tight text-slate-900 sm:text-xl">SeedanceChat</h1>
                  <p className="truncate text-xs text-slate-500 sm:text-sm">{modeMeta.subtitle}</p>
                </div>
              </div>

              <div className="flex flex-col items-end gap-2 sm:flex-row sm:items-center sm:gap-3">
                <div className="flex rounded-2xl border border-slate-200 bg-slate-50 p-1 shadow-inner shadow-white/60">
                  <button onClick={() => switchMode("drama")} className={`rounded-2xl px-3 py-1.5 text-xs font-medium transition sm:px-4 sm:py-2 sm:text-sm ${activeConversationMode === "drama" ? "bg-white text-slate-900 shadow-sm" : "text-slate-500 hover:text-slate-800"}`}>短剧</button>
                  <button onClick={() => switchMode("ad")} className={`rounded-2xl px-3 py-1.5 text-xs font-medium transition sm:px-4 sm:py-2 sm:text-sm ${activeConversationMode === "ad" ? "bg-white text-slate-900 shadow-sm" : "text-slate-500 hover:text-slate-800"}`}>广告</button>
                </div>
                <button onClick={clearConversation} className="rounded-full border border-slate-200 bg-white px-3 py-1.5 text-xs font-medium text-slate-600 shadow-sm hover:border-slate-300 hover:text-slate-900 sm:px-4 sm:py-2 sm:text-sm">清空对话</button>
                <div className="flex items-center gap-2 rounded-full border border-slate-200 bg-white px-3 py-1.5 text-xs text-slate-600 shadow-sm sm:py-2 sm:text-sm">
          ...
          <main className="flex-1 overflow-y-auto px-4 py-4 sm:px-6 sm:py-6">
            ...
            {isThinking && (
              <div className="mx-auto mt-2 flex max-w-5xl justify-start">
                <div className="flex items-center gap-2 rounded-full border border-sky-200 bg-white px-3 py-2 text-xs text-slate-600 shadow-sm shadow-sky-100/70 sm:px-4 sm:text-sm">
          ...
          {uploadedFiles.length > 0 && (
            <div className="border-t border-slate-200 bg-white/80 px-4 py-3 backdrop-blur-md sm:px-6">
          ...
          <div className="border-t border-slate-200 bg-white/85 px-4 py-3 backdrop-blur-md sm:px-6 sm:py-4">
            <div className="mx-auto flex max-w-5xl items-end gap-2 rounded-[24px] border border-slate-200 bg-white p-2.5 shadow-lg shadow-slate-200/60 sm:gap-3 sm:rounded-[28px] sm:p-3">
              <input
                type="file"
                ref={fileInputRef}
                className="hidden"
              ...
              <button
                ...
                disabled={uploading}
                className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-slate-100 text-lg text-slate-600 transition hover:bg-slate-200 hover:text-slate-900 disabled:opacity-50 sm:h-12 sm:w-12"
              ...
              <textarea
                ...
                rows={1}
                className="min-h-[44px] flex-1 resize-none rounded-2xl bg-slate-50 px-3 py-2.5 text-sm text-slate-800 outline-none placeholder:text-slate-400 focus:ring-2 focus:ring-sky-200 sm:min-h-[48px] sm:px-4 sm:py-3"
              />
              <button
                onClick={() => handleSend()}
                disabled={(!input.trim() && !uploadedFiles.length) || !isConnected}
                className="flex h-11 min-w-[52px] shrink-0 items-center justify-center rounded-2xl bg-slate-900 px-3 text-white shadow-lg shadow-slate-300/50 transition hover:bg-slate-800 disabled:bg-slate-200 disabled:text-slate-400 disabled:shadow-none sm:h-12 sm:min-w-[60px] sm:px-4"
              >
                ➤
              </button>
            </div>
